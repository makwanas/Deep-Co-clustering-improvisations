...
import core.general.pretrain_conv_autoencoder as ae
...
class KddcupPaeGmm:
    ...
    def eval(self, tru, pre):
        ...
        for i in range(nclu_pre):
            ...
            correct += max(np.bincount(tru[idx].T[0].astype(np.int32)))
    ...
    def run(self, data, train_epochs, pretrain_epochs):
        ...
        #train_z_col = tf.reshape(train_z_col, shape=[166, 256])
        ...
        for i in range(len(train_var_list)):
            ...
            train_step_i = tf.train.AdamOptimizer(1e-3).minimize(obj_oa_pretrain)
        ...
        for i in range(len(train_var_list_col)):
            ...
            train_col_step_i = tf.train.AdamOptimizer(1e-3).minimize(obj_oa_col_pretrain)
...
'MI_org':self.MI_org, 'MI_red':self.MI_red, 'Ur_max_idx':self.Ur_max_idx, 'Uc_max_idx':self.Uc_max_idx}
            ...
            #print(fetch)
            #print("train_norm_x:",train_norm_x)
            #print("train_norm_x_col:", train_norm_x_col)
...
#         sio.savemat('.../DeepCC'+'_'+'coil20'+'_'+'prdlabel.mat', {'prd_lab':prd_lab, 'prd_lab_col':prd_lab_col})
        ...
        sess.close()
        return RR_acc, RR_nmi, RR_loss, RR_loss_col
