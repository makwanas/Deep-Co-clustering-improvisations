...
class KddcupDataP10:
    ...
    def get_clean_training_testing_data(self, ratio):
        ...
        self.train_data = np.zeros([self.num_train_points, 120], dtype=np.float32)
        self.test_data = np.zeros([self.num_test_points, 120], dtype=np.float32)
        self.test_label = np.zeros([self.num_test_points, 1], dtype=np.float32)
        head = 0
        for i in range(self.num_assign_points):
            pl = float(self.xy[i][120])
            if pl < 1:
