...
class PretrainAutoencoder:
    def __init__(self, config, num_drop_out):
        ...
        # Encode
        for i in range(len(self.num_dim)-1):
            w = initializer([self.num_dim[i], self.num_dim[i+1]])
            b = tf.constant(0.1, shape=[self.num_dim[i+1]], dtype=tf.float64)
            self.wi += [tf.Variable(w)]
            self.bi += [tf.Variable(b)]
        # Decode
        for i in range(len(self.num_dim)-1 , 0, -1):
            w = initializer([self.num_dim[i], self.num_dim[i-1]])
            b = tf.constant(0.1, shape=[self.num_dim[i-1]], dtype=tf.float64)
            self.wi += [tf.Variable(w)]
            self.bi += [tf.Variable(b)]
