...
import core.general.pretrain_sparse_autoencoder as ae
...
class KddcupPaeGmm:
    ...
    def MI_Table(self, T):
        ...
        MI_temp = tf.log((T + 0.1 ** 15) / (T_xy + 0.1 ** 15)) / tf.log(tf.constant(2, tf.float32))
    ...
    def run(self, data, train_epochs, pretrain_epochs):
        ...
        # Setup
        train_x_v = tf.placeholder(dtype=tf.float32, shape=[train_norm_x.shape[0], train_norm_x.shape[1]])
        train_x_v_col = tf.placeholder(dtype=tf.float32, shape=[train_norm_x_col.shape[0], train_norm_x_col.shape[1]])
        keep_prob = tf.placeholder(tf.float32)

        # Autoencoder
        train_z, train_error, train_var_list, train_l2_reg, train_reg = self.autoencoder.run(train_x_v, keep_prob)
        train_z_col, train_error_col, train_var_list_col, train_l2_reg_col, train_reg_col = self.autoencoder_col.run(
            train_x_v_col, keep_prob)
        ...
        for i in range(len(train_var_list_col)):
            ...
            train_col_step_i = tf.train.AdamOptimizer(1e-4).minimize(obj_oa_col_pretrain,
                                                                     var_list=train_var_list_col[i])
        ...
        loss, pen_dev, likelihood, p_z, x_t, p_t, z_p, z_t, mixture_mean, mixture_dev, mixture_cov, mixture_dev_det = self.e_net.run(
            train_z, keep_prob)

        loss_col, pen_dev_col, likelihood_col, p_z_col, x_t_col, p_t_col, z_p_col, z_t_col, mixture_mean_col, mixture_dev_col, mixture_cov_col, mixture_dev_det_col = self.e_net_col.run(
            train_z_col, keep_prob)
        ...
        for k in range(len(pretrain_step)):
            ...
            for i in range(pretrain_epochs):  # epoch_tot
                ...
                if (i + 1) % 1 == 0:
                    ...
                    print(f'Train col object ====================================================> {train_obj.shape}')
        ...
        for k in range(len(pretrain_col_step)):
            ...
            for i in range(pretrain_epochs):  # epoch_tot_col
                ...
                if (i + 1) % 1 == 0:
                    train_col_obj = obj_col_k.eval(
                        feed_dict={keep_prob: dropout_pretrain, train_x_v_col: train_norm_x_col})
        ...
        for k in range(epoch_tot):
            ...
            if k < 1500:
                train_step_1.run(
                    feed_dict={keep_prob: dropout, train_x_v: train_norm_x, train_x_v_col: train_norm_x_col})
            if k >= 1500:
                train_step_2.run(
                    feed_dict={keep_prob: dropout, train_x_v: train_norm_x, train_x_v_col: train_norm_x_col})
            ...
            fetch = {'obj_oa': obj_oa, \
                     ...
                     'error_oa_col': error_oa_col, 'train_l2_reg_col': train_l2_reg_col, 'loss_col': loss_col,
                     'pen_dev_col': pen_dev_col, \
                     ...
                     'mixture_dev_col': mixture_dev_col, 'mixture_cov_col': mixture_cov_col,
                     'mixture_dev_det_col': mixture_dev_det_col, \
                     'x_t': x_t, 'p_t': p_t, 'z_p': z_p, 'z_t': z_t, \
                     'x_t_col': x_t_col, 'p_t_col': p_t_col, 'z_p_col': z_p_col, 'z_t_col': z_t_col, \
                     'MI_org': self.MI_org, 'MI_red': self.MI_red, 'Ur_max_idx': self.Ur_max_idx,
                     'Uc_max_idx': self.Uc_max_idx}
            ...
            print("Epoch %d: obj_col %g; error_oa_col %g; train_l2_reg_col %g; loss_col %g; pen_dev_col %g;"
                  % (k + 1, RR['obj_oa_col'], RR['error_oa_col'], RR['train_l2_reg_col'], RR['loss_col'],
                     RR['pen_dev_col']))
